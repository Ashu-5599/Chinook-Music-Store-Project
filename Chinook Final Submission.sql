...
set 
    ...
    -- -----------------------------------------------------Remove null values in employee table -----------------------------------------------------------------
update employee
set 
...
-- Answer for Q.7 7.	Are there any patterns or trends in customer purchasing behavior (e.g., frequency of purchases, preferred payment methods, average order value)?--

SELECT 
    c.customer_id,
    c.first_name || ' ' || c.last_name AS customer_name,
    COUNT(i.invoice_id) AS total_purchases,  -- Purchase frequency
    AVG(i.total) AS avg_order_value,  -- Average order value
    MAX(i.total) AS max_order_value,  -- Highest order value
    MIN(i.total) AS min_order_value,  -- Lowest order value
    (SUM(i.total) / COUNT(DISTINCT i.invoice_date)) AS avg_daily_spend  -- Avg spend per day
FROM customer c
JOIN invoice i ON c.customer_id = i.customer_id
GROUP BY c.customer_id
ORDER BY total_purchases DESC
LIMIT 10;  -- Get top 10 customers by purchase frequency


-- Answer for Q.8  What is the customer churn rate?  ----------------------------------------------------------------------------------------------------------
...
-- Answer for Q.10 Find customers who have purchased tracks from at least 3 different genres -------------------------------------------------------------------
...
-- Q.11  Answer for Rank genres based on their sales performance in the USA-----------------------------------------------------------------------------------------
...
-- Answer for Q.12	Identify customers who have not made any purchase in the last 3 months------------------------------------------------------
...
/* ----------------------Q.1 	Recommend the three albums from the new record label that should be prioritised  -------------------------------------------
...
/*Q.3.	Customer Purchasing Behavior Analysis: How do the purchasing habits 
...
/*Q.6.	Customer Risk Profiling: Based on customer profiles (age, gender, location, purchase history), 
...
/*Q 7.	Customer Lifetime Value Modeling: How can you leverage customer data (tenure, purchase history, engagement) to predict the lifetime value of
...
-- ----------------------------Procedure to get lowest expenditure demographics------------------------------------------------------------------------------------------------------------------------------------------------

USE chinook;
DELIMITER //

CREATE DEFINER=`root`@`localhost` PROCEDURE `total_expenditure_by_region`()
BEGIN
    -- top 5 countries with the lowest total expenditure
    ...
    --  top 5 states with the lowest total expenditure
    SELECT i.billing_state, 
           SUM(il.quantity * il.unit_price) AS total_cost 
    FROM invoice i 
    JOIN invoice_line il ON i.invoice_id = il.invoice_id
    GROUP BY i.billing_state
    ORDER BY total_cost ASC 
    LIMIT 5;

    --  top 5 cities with the lowest total expenditure
...
-- -- --- ----- procedure 1 to the top state country and city


DELIMITER $$

CREATE DEFINER=`root`@`localhost` PROCEDURE `total_revenue_for_country_state_city`()  
BEGIN  
